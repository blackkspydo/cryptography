...
void getPrimitive()
{
    ...
    {
        if (not_primitive == 0)
        {                            // to check if the potential primitive is already found not primitive after at least one loop check
    ...
    printf("\n");
    if (not_primitive == 0)
    { // after the frequency check, if the not_primitive is still 0 then the number is primitive.
